...
| 模式 | 优势 | 适用场景 |
|------|------|----------|
| 本地模式 | 完全离线，保护隐私，节省 API 费用 | 处理敏感文档、频繁问答、无网络环境 |
| 在线模式 | 答案质量更高，响应更快，无需本地算力 | 追求最佳效果、有稳定网络、偶发使用 |

*本地模式使用 Qwen3-4B GGUF 量化模型（2.5GB），4GB 显存即可运行。*
*在线模式支持 OpenAI-compatible API，可接入 GPT-4o、Claude 等模型。*
...
- **GPU 加速** — 自动检测 NVIDIA CUDA，支持 GPU 推理（如无 GPU 则使用 CPU）
...
| 状态管理 | Pinia | 轻量响应式 |
| 本地 LLM | llama.cpp (llama-server) | Qwen3-4B-Q5_K_M GGUF |
...
如需手动配置模型路径或在本地/在线模式间切换，可在设置页面修改。
